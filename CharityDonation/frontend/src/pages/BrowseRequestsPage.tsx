  ...
  return (
    <div className="container" style={{ maxWidth: '1200px' }}>
      <div style={{ marginBottom: '2.5rem' }}>
        <h1 style={{ fontSize: '2.5rem', fontWeight: 800, color: '#0f172a', marginBottom: '0.25rem' }}>Browse All Requests</h1>
        <p className="muted" style={{ fontSize: '1.1rem' }}>Showing active community requests</p>
        
        <div style={{ display: 'flex', gap: '1rem', marginTop: '1.5rem', flexWrap: 'wrap' }}>
          <div className="input-wrapper" style={{ flex: 1, minWidth: '250px', maxWidth: '400px' }}>
            <Search className="input-icon" size={18} />
            <input placeholder="Search requests..." style={{ borderRadius: '9999px', padding: '0.6rem 1rem 0.6rem 2.5rem' }} />
          </div>
      ...
      {isLoading && (
        <div style={{ textAlign: 'center', padding: '3rem' }}>
          <div className="muted">Loading community requests...</div>
        </div>
      )}
        ...
        {data?.map((r) => (
          <div className="dashboard-card" key={r.id} style={{ padding: '1.25rem' }}>
            ...
            <div className="donor-block">
              <div className="donor-avatar" style={{ background: 'linear-gradient(135deg, #3b82f6, #2563eb)' }}>
                {r.requester_name ? r.requester_name.charAt(0).toUpperCase() : "R"}
              </div>
              <span style={{ fontSize: '0.9rem', color: '#334155' }}>
                requested by <strong style={{ color: '#0f172a' }}>{r.requester_name}</strong>
              </span>
            </div>
